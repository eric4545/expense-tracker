...
export class ExpenseCalculationService {
   ...
   */
  static getTotalPaid(expenses, member) {
   ...
   */
  static getTotalShouldPay(expenses, member) {
    let total = 0
   ...
   */
  static getBalance(expenses, member) {
    const paid = ExpenseCalculationService.getTotalPaid(expenses, member)
    const shouldPay = ExpenseCalculationService.getTotalShouldPay(
      expenses,
      member
    )
   ...
   */
  static getAllBalances(expenses, members) {
    const balances = {}
    for (const member of members) {
      balances[member] = ExpenseCalculationService.getBalance(expenses, member)
    }
    return balances
  }
   ...
   */
  static getPaymentPlan(expenses, members) {
    const payments = []
    const balances = ExpenseCalculationService.getAllBalances(expenses, members)
   ...
   */
  static getCrossTableAmount(expense, member) {
   ...
   */
  static getCrossPaidAmount(expenses, payer, receiver) {
    if (payer === receiver) return 0

    return expenses.reduce((sum, expense) => {
      if (expense.paidBy === payer && expense.splitWith.includes(receiver)) {
        return (
          sum + ExpenseCalculationService.getCrossTableAmount(expense, receiver)
        )
      }
      return sum
    }, 0)
